...
pub fn parse(bitcode: &[u8]) -> anyhow::Result<Module> {
    unsafe {
...
unsafe fn value_name(v: LLVMValueRef) -> String {
    let mut len = 0;
    let mut p = LLVMGetValueName2(v, &mut len);
    str::from_utf8_unchecked(std::slice::from_raw_parts(p as _, len)).to_owned()
}
